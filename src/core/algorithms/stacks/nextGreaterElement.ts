 ...
 */
export const nextGreaterElement: IAlgorithm<ArrayInput> = {
  ...
  *run(input: ArrayInput): Generator<AlgoEvent, void, unknown> {
    ...
    yield createEvent.message(`Finding next greater element for each element in [${arr.join(', ')}]`, 'info', 0);
    ...
    for (let i = 0; i < n; i++) {
      ...
      while (stack.length > 0 && arr[i] > arr[stack[stack.length - 1]]) {
        ...
        const idx = stack[stack.length - 1];  // Get element without removing yet
        yield createEvent.message(`arr[${i}]=${arr[i]} > arr[${idx}]=${arr[idx]}, so NGE[${idx}] = ${arr[i]}`, 'explanation');
        ...
        yield createEvent.set(idx, arr[i]);
      }

      // Push current index
      yield createEvent.highlight([8]);
      yield createEvent.message(`Pushing arr[${i}]=${arr[i]} onto stack`, 'explanation');
    ...
    // Mark remaining elements as no greater
    yield createEvent.pointer([], []);
    yield createEvent.message(`Remaining stack elements have no greater element (NGE = -1)`, 'explanation');

    yield createEvent.highlight([9]);
    yield createEvent.message(`Result: [${result.join(', ')}]`, 'info');
    yield createEvent.result('indices', result, `Next Greater Elements: [${result.join(', ')}]`);
  },
};
