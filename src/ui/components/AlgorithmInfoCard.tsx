...
export function AlgorithmInfoCard({ info }: AlgorithmInfoCardProps) {
  // Return null if info is missing required properties
  ...
  return (
    <div className="space-y-4 p-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border-primary)]">
      ...
      {/* Best For / Avoid When */}
      {(info.bestFor?.length > 0 || info.avoidWhen?.length > 0) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {info.avoidWhen && info.avoidWhen.length > 0 && (
      ...
      {/* Optimization Tips */}
      {info.optimizationTips && info.optimizationTips.length > 0 && (
